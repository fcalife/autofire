...
function GameMode:_InitGameMode()
	...
	GameRules:SetPreGameTime((IsInToolsMode() and 5) or 15)
...
function GameMode:_CaptureGameMode()
	if mode == nil then
		...
		for level = 2, 100 do
			xp_table[level] = xp_table[level - 1] + 100
		end
		...
		local force_pick_hero = "npc_dota_hero_windrunner"
		if force_pick_hero then	mode:SetCustomGameForceHero(force_pick_hero) end
		...
		mode:SetMaximumAttackSpeed(100000)
		-- What should we use for the minimum attack speed?
		mode:SetMinimumAttackSpeed(10)
